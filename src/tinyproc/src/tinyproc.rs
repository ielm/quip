...
//! fn schedule_function(proc: TinyProc) {;}
//!
...
//! let panic_recoverable = TinyProc::recoverable(
//!     future,
//!     schedule_function,
//!     proc_stack
//! );
...
unsafe impl Send for TinyProc {}
unsafe impl Sync for TinyProc {}
...
impl TinyProc {
    ...
    ///
    /// Schedule the tinyproc for runnning on the thread.
    pub fn run(self) {
        let ptr = self.raw_proc.as_ptr();
        let pdata = ptr as *const ProcData;
        mem::forget(self);
    ...
    ///
    /// Cancel polling the tinyproc's inner future, thus cancelling th proc itself.
    pub fn cancel(&self) {
        let ptr = self.raw_proc.as_ptr();
        let pdata = ptr as *const ProcData;
...
impl Debug for TinyProc {
    fn fmt(&self, fmt: &mut Formatter) -> fmt::Result {
        let ptr = self.raw_proc.as_ptr();
        let pdata = ptr as *const ProcData;

        fmt.debug_struct("TinyProc")
