...
///
/// A proc is considered to be scheduled whenever its `tinyproc` reference exists. It is in scheduled
...
///
/// 1. It gets cancelled by `tinyproc::cancel()` or `ProcHandle::cancel()`.
...
/// proc references (`tinyproc` and `Waker`s) are tracked by the reference count.
...
///
/// Note that the reference counter only tracks the `tinyproc` and `Waker`s. The `ProcHandle` is
