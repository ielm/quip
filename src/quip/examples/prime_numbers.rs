...
// `serve_prime_numbers` is the quip child's behavior.
// a more general waltkhrough of the msg! macro can be found in the fibonacci example.
async fn serve_prime_numbers(ctx: QuipContext) -> Result<(), ()> {
...
// _on the client side_ to generate load on the quip.
...
fn main() {
    ...
    let supervisor: SupervisorRef = Quip::supervisor(|sp| {
