...
/// algorithm with quip. We will use two groups of children, one will be
...
fn main() {
    ...
    // We need quip to run our program
    Quip::init();
    // We create the supervisor and we add both groups on it
    Quip::supervisor(caller_supervisor)
        .and_then(|_| Quip::supervisor(receiver_supervisor))
        .expect("Couldn't create supervisor chain.");
    // We are starting the Quip program now
    Quip::start();
    // We are waiting until the Quip has stopped or got killed
    Quip::block_until_stopped();
}

fn caller_supervisor(supervisor: Supervisor) -> Supervisor {
    // We create a new children, it wrap the Quip::Children method to add it on the supervisor
    ...
    children
        ...
        .with_exec(move |ctx: QuipContext| {
            async move {
                ...
                // We stop quip here, because we don't have more data to send
                Quip::stop();
                Ok(())
            }
        })
