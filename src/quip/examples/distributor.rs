...
fn run() -> AnyResult<()> {
    ...
    .map_err(|_| anyhow!("couldn't setup the quip"))?;
...
async fn organize_the_event(ctx: QuipContext) -> Result<(), ()> {
    loop {
...
async fn be_interested_in_the_conference(ctx: QuipContext) -> Result<(), ()> {
    loop {
        MessageHandler::new(ctx.recv().await?)
