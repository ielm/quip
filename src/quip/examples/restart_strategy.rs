...
/// with a custom strategy, provided by the quip crate. The supervisor will spawn
...
fn main() {
    ...
    Quip::supervisor(supervisor).expect("Couldn't create the supervisor.");
...
fn supervisor(supervisor: Supervisor) -> Supervisor {
    ...
    // By default the quip's supervisors are always trying to restart
...
fn failed_actors_group(children: Children) -> Children {
    ...
    // with panic.
    children.with_exec(move |_ctx: QuipContext| async move {
