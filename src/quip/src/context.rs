...
pub const NIL_ID: QuipId = QuipId(Uuid::nil());

#[derive(Hash, Eq, PartialEq, Debug, Clone)]
/// An identifier used by supervisors, children groups and
/// their elements to identify themselves, using a v4 UUID.
///
/// A `QuipId` is unique to its attached element and is
/// reset when it is restarted. A special `QuipId` exists
...
/// # }
/// ```
pub struct QuipId(pub(crate) Uuid);
...
///             // this child was created using `Quip::children`,
...
pub(crate) struct ContextState {
    ...
    stats: Arc<AtomicU64>,
    #[cfg(feature = "scaling")]
    actor_stats: Arc<LOTable<QuipId, u32>>,
}
...
impl QuipContext {
    ...
    /// element that is linked to this `QuipContext`.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// of the element that is linked to this `QuipContext`.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    /// `QuipContext` if it isn't the system supervisor
    ...
    pub async fn try_recv(&self) -> Option<SignedMessage> {
        ...
        trace!("QuipContext({}): Trying to receive message.", self.id);

        if let Some(msg) = self.state.pop_message() {
            trace!("QuipContext({}): Received message: {:?}", self.id, msg);
            Some(msg)
        } else {
            trace!("QuipContext({}): Received no message.", self.id);
            None
        }
    }

    /// Retrieves asynchronously a message received by the element
    /// this `QuipContext` is linked to and waits (always
    ...
    pub async fn recv(&self) -> Result<SignedMessage, ()> {
        debug!("QuipContext({}): Waiting to receive message.", self.id);
        loop {
            if let Some(msg) = self.state.pop_message() {
                trace!("QuipContext({}): Received message: {:?}", self.id, msg);
                return Ok(msg);
    ...
    /// Retrieves asynchronously a message received by the element
    /// this `QuipContext` is linked to and waits until `timeout` (always
    /// asynchronously) for one if none has been received yet.
    ///
    ...
    pub async fn try_recv_timeout(&self, timeout: Duration) -> Result<SignedMessage, ReceiveError> {
        debug!(
            "QuipContext({}): Waiting to receive message within {} milliseconds.",
            self.id,
            timeout.as_millis()
        );
        futures::select! {
    ...
    /// Returns [`RefAddr`] of the current `QuipContext`
    ///
    /// # Example
    ///
    /// ```rust
    ...
    pub fn ask<M: Message>(&self, to: &RefAddr, msg: M) -> Result<Answer, M> {
        ...
        let (msg, answer) = QuipMessage::ask(msg, self.signature());
        let env = Envelope::new_with_sign(msg, self.signature());
        // FIXME: panics?
        to.sender()
            .unbounded_send(env)
...
impl ContextState {
    ...
    #[cfg(feature = "scaling")]
    pub(crate) fn set_actor_stats(&mut self, actor_stats: Arc<LOTable<QuipId, u32>>) {
    ...
    #[cfg(feature = "scaling")]
    pub(crate) fn actor_stats(&self) -> Arc<LOTable<QuipId, u32>> {
