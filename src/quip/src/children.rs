...
use crate::context::{ContextState, QuipContext, QuipId};
...
pub struct Children {
    ...
    launched: FxHashMap<QuipId, (Sender, RecoverableHandle<()>)>,
    ...
    // stats, etc.
    helper_actors: FxHashMap<QuipId, (Sender, RecoverableHandle<()>)>,
}
...
impl Children {
    ...
    ///     let children_id: &QuipId = children.id();
    ///     // ...
    /// # children
    /// }).expect("Couldn't create the children group.");
    /// #
    ...
    /// Sets the closure taking a [`QuipContext`] and returning a
    ...
    /// # Arguments
    ///
    /// * `init` - The closure taking a [`QuipContext`] and returning
    ...
    pub fn with_dispatcher(mut self, dispatcher: Dispatcher) -> Self {
        self.dispatchers.push(Arc::new(dispatcher));
        self
    }
    ...
    fn get_heartbeat_fut(&self) -> Init {
        ...
        let exec_fut = move |ctx: QuipContext| async move {
    ...
    async fn handle_stopped_child(&mut self, id: &QuipId) -> Result<(), ()> {
        // FIXME: Err if false?
        if self.launched.contains_key(id) {
            debug!("Children({}): Child({}) stopped.", self.id(), id);
            self.drop_child(id);

            let msg = QuipMessage::finished_child(id.clone(), self.bcast.id().clone());
    ...
    async fn handle_faulted_child(&mut self, id: &QuipId) -> Result<(), ()> {
        // FIXME: Err if false?
        if self.launched.contains_key(id) {
    ...
    fn request_restarting_child(&mut self, id: &QuipId, parent_id: &QuipId) {
        if parent_id == self.bcast.id() && self.launched.contains_key(id) {
            let parent_id = self.bcast.id().clone();
            let msg = QuipMessage::restart_required(id.clone(), parent_id);
            let env = Envelope::new(msg, self.bcast.path().clone(), self.bcast.sender().clone());
            self.bcast.send_parent(env).ok();
        }
    }

    fn restart_child(&mut self, old_id: &QuipId, old_state: Arc<Pin<Box<ContextState>>>) {
        let parent = Parent::children(self.as_ref());
        let bcast = Broadcast::new(parent, QuipPathElement::Child(old_id.clone()));
        ...
        let msg = QuipMessage::set_state(old_state);
        let env = Envelope::new(msg, self.bcast.path().clone(), self.bcast.sender().clone());
        self.bcast.send_child(&id, env);

        let msg = QuipMessage::apply_callback(CallbackType::AfterRestart);
        let env = Envelope::new(msg, self.bcast.path().clone(), self.bcast.sender().clone());
        self.bcast.send_child(&id, env);
    ...
    fn drop_child(&mut self, id: &QuipId) {
        debug!(
    ...
    async fn handle(&mut self, envelope: Envelope) -> Result<(), ()> {
        match envelope {
            ...
            Envelope {
                msg: QuipMessage::RestoreChild { id, state },
                ..
            } => self.restart_child(&id, state),
            Envelope {
                msg: QuipMessage::DropChild { id },
                ..
            } => self.drop_child(&id),
            Envelope {
    ...
    pub(crate) fn launch_child(&mut self) {
        let name = self.name();
        let parent = Parent::children(self.as_ref());
        let bcast = Broadcast::new(parent, QuipPathElement::Child(QuipId::new()));
        ...
        let parent_id = self.bcast.id().clone();
        let msg = QuipMessage::instantiated_child(parent_id, id.clone(), state.clone());
        let env = Envelope::new(msg, self.bcast.path().clone(), self.bcast.sender().clone());
        self.bcast.send_parent(env).ok();
    ...
    pub(crate) fn launch_heartbeat(&mut self) {
        ...
        let ctx = QuipContext::new(id, child_ref.clone(), children, supervisor, state.clone());
