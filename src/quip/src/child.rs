...
impl Init {
    ...
    where
        C: Fn(QuipContext) -> F + Send + 'static,
        F: Future<Output = Result<(), ()>> + Send + 'static,
    {
        let init = Box::new(move |ctx: QuipContext| {
            let fut = init(ctx);
...
impl Child {
    ...
    fn stack(&self) -> ProcStack {
        ...
        ProcStack::default().with_after_panic(move |_state: &mut EmptyProcState| {
            ...
            let id = id.clone();
            let msg = QuipMessage::restart_required(id, parent.id().clone());
    ...
    fn faulted(&mut self) {
        ...
        let msg = QuipMessage::restart_required(self.id().clone(), parent.id().clone());
    ...
    async fn handle(&mut self, env: Envelope) -> Result<(), ()> {
        match env {
            ...
            Envelope {
                msg: QuipMessage::ApplyCallback(callback_type),
                ..
            ...
            Envelope {
                msg: QuipMessage::SetState { state },
                ..
            } => {
            ...
            // FIXME
            Envelope {
                msg: QuipMessage::Stopped { .. },
                ..
            } => unimplemented!(),
            // FIXME
            Envelope {
                msg: QuipMessage::Faulted { .. },
                ..
            } => unimplemented!(),
            Envelope {
