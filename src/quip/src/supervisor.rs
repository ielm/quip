...
use crate::message::{Deployment, Message, QuipMessage};
...
/// supervising children groups created via [`Quip::children`].
///
/// # Example
///
/// ```rust
...
/// ```
///
/// [`Quip::children`]: crate::Quip::children
...
pub struct Supervisor {
    ...
    // The key is the Children's QuipId and the values are
    // represented as Child instances with the same executed closure.
    tracked_groups: FxHashMap<QuipId, Vec<TrackedChildState>>,
    // Hold the insertion order of the childs.
    tracked_groups_order: FxHashMap<QuipId, usize>,
    ...
    launched: FxHashMap<QuipId, (usize, RecoverableHandle<Supervised>)>,
...
enum RestartedElement {
    Supervisor(QuipId),
    Child { id: QuipId, parent_id: QuipId },
}

#[derive(Debug)]
enum ActorSearchMethod {
    OneActor { id: QuipId, parent_id: QuipId },
    FromActor { id: QuipId, parent_id: QuipId },
    All,
}
...
impl Supervisor {
    ...
    /// #
    /// Quip::supervisor(|sp| {
    ///     let supervisor_id: &QuipId = sp.id();
    ///     // ...
    /// # sp
    /// }).expect("Couldn't create the supervisor.");
    ///
        ...
        for object in objects {
            match object {
                RestartedElement::Supervisor(supervisor_id) => {
                    let msg = QuipMessage::restart_subtree();
                    let env =
                ...
                RestartedElement::Child { id, parent_id } => {
                    ...
                    let msg = match restart_required {
                        true => {
                            ...
                            QuipMessage::restore_child(id, state)
                        }
                        false => {
                            self.remove_child(&id.clone(), &parent_id.clone());
                            QuipMessage::drop_child(id)
                        }
                    };
                    let restart_strategy = self.restart_strategy.clone();

                    restart_futures.push_back(async move {
                        if restart_required {
    ...
    fn remove_child(&mut self, id: &QuipId, parent_id: &QuipId) {
        let index = match self.tracked_groups_order.get(id) {
            Some(index) => *index,
            None => return,
        };
    ...
    async fn recover(&mut self, id: QuipId, parent_id: QuipId) -> Result<(), ()> {
        debug!(
    ...
    async fn cleanup_supervised_object(&mut self, id: QuipId) {
        // FIXME: Err if None?
        if let Some((_, launched)) = self.launched.remove(&id) {
    ...
    async fn recover_supervised_object(&mut self, id: QuipId, parent_id: QuipId) -> Result<(), ()> {
    ...
    async fn handle(&mut self, env: Envelope) -> Result<(), ()> {
        match env {
            ...
            // FIXME
            Envelope {
                msg: QuipMessage::Prune { .. },
                ..
            } => unimplemented!(),
            Envelope {
                msg: QuipMessage::SuperviseWith(strategy),
                ..
            } => {
                debug!(
                    "Supervisor({}): Setting strategy: {:?}",
            ...
            Envelope {
                msg: QuipMessage::RestartRequired { id, parent_id },
                ..
            } => {
            ...
            Envelope {
                msg: QuipMessage::FinishedChild { id, parent_id },
                ..
...
impl SupervisorRef {
    pub(crate) fn new(id: QuipId, sender: Sender, path: Arc<QuipPath>) -> Self {
    ...
    /// # Quip::init();
    /// #
    /// let supervisor_ref = Quip::supervisor(|sp| {
    ///     // ...
    ///     # sp
    /// }).expect("Couldn't create the supervisor.");
    ///
    /// let supervisor_id: &QuipId = supervisor_ref.id();
    /// #
    /// # Quip::start();
    /// # Quip::stop();
    /// # Quip::block_until_stopped();
    ...
    /// # Quip::init();
    /// #
    /// # let mut parent_ref = Quip::supervisor(|sp| sp).unwrap();
    ...
    /// # Quip::init();
    /// #
    /// # let sp_ref = Quip::supervisor(|sp| sp).unwrap();
    ...
    pub fn strategy(&self, strategy: SupervisionStrategy) -> Result<(), ()> {
        ...
        let msg = QuipMessage::supervise_with(strategy);
...
impl TrackedChildState {
    fn new(id: QuipId, state: Arc<Pin<Box<ContextState>>>) -> Self {
        TrackedChildState {
            id,
            state,
            restarts_counts: 0,
