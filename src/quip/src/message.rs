...
use crate::context::{ContextState, QuipId};
...
#[derive(Debug)]
/// A message returned by [`QuipContext::recv`] or
/// [`QuipContext::try_recv`] that should be passed to the
...
/// [`QuipContext::try_recv`]: crate::context::QuipContext::try_recv
...
impl QuipMessage {
    ...
    pub(crate) fn deploy_supervisor(supervisor: Supervisor) -> Self {
        ...
        QuipMessage::Deploy(deployment.into())
    }
    ...
    pub(crate) fn supervise_with(strategy: SupervisionStrategy) -> Self {
        QuipMessage::SuperviseWith(strategy)
    }

    pub(crate) fn apply_callback(callback_type: CallbackType) -> Self {
        QuipMessage::ApplyCallback(callback_type)
    }
    ...
    pub(crate) fn ask<M: Message>(msg: M, sign: RefAddr) -> (Self, Answer) {
        ...
        (QuipMessage::Message(msg), answer)
    }

    pub(crate) fn restart_required(id: QuipId, parent_id: QuipId) -> Self {
        QuipMessage::RestartRequired { id, parent_id }
    }

    pub(crate) fn finished_child(id: QuipId, parent_id: QuipId) -> Self {
        QuipMessage::FinishedChild { id, parent_id }
    }

    pub(crate) fn restart_subtree() -> Self {
        QuipMessage::RestartSubtree
    }

    pub(crate) fn restore_child(id: QuipId, state: Arc<Pin<Box<ContextState>>>) -> Self {
        QuipMessage::RestoreChild { id, state }
    }

    pub(crate) fn drop_child(id: QuipId) -> Self {
        QuipMessage::DropChild { id }
    }

    pub(crate) fn set_state(state: Arc<Pin<Box<ContextState>>>) -> Self {
        QuipMessage::SetState { state }
    }
    ...
    pub(crate) fn faulted(id: QuipId) -> Self {
        QuipMessage::Faulted { id }
    }

    pub(crate) fn heartbeat() -> Self {
        QuipMessage::Heartbeat
    }

    pub(crate) fn try_clone(&self) -> Option<Self> {
        trace!("{:?}: Trying to clone.", self);
        let clone = match self {
            QuipMessage::Start => QuipMessage::start(),
            QuipMessage::Stop => QuipMessage::stop(),
            QuipMessage::Kill => QuipMessage::kill(),
            // FIXME
            QuipMessage::Deploy(_) => unimplemented!(),
            QuipMessage::Prune { id } => QuipMessage::prune(id.clone()),
            QuipMessage::SuperviseWith(strategy) => QuipMessage::supervise_with(strategy.clone()),
            QuipMessage::ApplyCallback(callback_type) => {
                QuipMessage::apply_callback(callback_type.clone())
            }
            QuipMessage::InstantiatedChild {
                parent_id,
                child_id,
                ...
                QuipMessage::instantiated_child(parent_id.clone(), child_id.clone(), state.clone())
            }
            QuipMessage::Message(msg) => QuipMessage::Message(msg.try_clone()?),
            QuipMessage::RestartRequired { id, parent_id } => {
                QuipMessage::restart_required(id.clone(), parent_id.clone())
            }
            QuipMessage::FinishedChild { id, parent_id } => {
                QuipMessage::finished_child(id.clone(), parent_id.clone())
            }
            QuipMessage::RestartSubtree => QuipMessage::restart_subtree(),
            QuipMessage::RestoreChild { id, state } => {
                QuipMessage::restore_child(id.clone(), state.clone())
            }
            QuipMessage::DropChild { id } => QuipMessage::drop_child(id.clone()),
            QuipMessage::SetState { state } => QuipMessage::set_state(state.clone()),
            QuipMessage::Stopped { id } => QuipMessage::stopped(id.clone()),
            QuipMessage::Faulted { id } => QuipMessage::faulted(id.clone()),
            QuipMessage::Heartbeat => QuipMessage::heartbeat(),
        };
    ...
    pub(crate) fn into_msg<M: Message>(self) -> Option<M> {
        if let QuipMessage::Message(msg) = self {
            msg.try_unwrap().ok()
        } else {
            None
        }
...
#[macro_export]
/// Matches a [`Msg`] (as returned by [`QuipContext::recv`]
/// or [`QuipContext::try_recv`]) with different types.
///
...
///     #
///     # Quip::start();
///     # Quip::broadcast(BCAST_MSG).unwrap();
///     # Quip::stop();
///     # Quip::block_until_stopped();
/// # }
/// ```
...
/// ```rust
/// # use quip::prelude::*;
/// # use quip::message::MessageHandler;
/// #
/// # #[cfg(feature = "tokio-runtime")]
/// # #[tokio::main]
/// # async fn main() {
