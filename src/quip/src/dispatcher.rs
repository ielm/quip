...
/// the Quip identifier as the key and the module name as the value.
pub type DispatcherMap = LOTable<ChildRef, String>;

/// Type alias for the recipients hashset.
/// Each key-value pair stores the Quip identifier as the key.
...
/// A generic implementation of the Quip dispatcher
///
...
impl Dispatcher {
    ...
    pub fn with_type(dispatcher_type: DispatcherType) -> Self {
        ...
        Self {
            dispatcher_type,
            handler: Box::<RoundRobinHandler>::default(),
...
impl From<String> for DispatcherType {
    fn from(val: String) -> Self {
        match val == DispatcherType::Anonymous.name() {
            true => DispatcherType::Anonymous,
            false => DispatcherType::Named(val),
        }
    }
}

#[derive(Debug)]
/// The global dispatcher of quip the cluster.
///
...
pub(crate) struct GlobalDispatcher {
    ...
    pub dispatchers: LOTable<DispatcherType, Arc<Dispatcher>>,
...
impl GlobalDispatcher {
    ...
    pub(crate) fn new() -> Self {
        GlobalDispatcher {
            ...
            distributors: Arc::new(RwLock::new(HashMap::new())), // TODO: switch to LOTable once lever implements write optimized granularity
    ...
    pub(crate) fn register_dispatcher(&self, dispatcher: &Arc<Dispatcher>) -> AnyResult<()> {
    ...
    pub(crate) fn remove_dispatcher(&self, dispatcher: &Arc<Dispatcher>) -> AnyResult<()> {
    ...
    pub(crate) fn register_distributor(&self, distributor: &Distributor) -> AnyResult<()> {
        ...
        if distributors.contains_key(distributor) {
            ...
            distributors.insert(*distributor, Box::<RoundRobinHandler>::default());
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_local_dispatcher_append_child_ref() {
        let instance = Dispatcher::default();
        let quip_id = QuipId::new();
        let (sender, _) = mpsc::unbounded();
        let path = Arc::new(QuipPath::root());
        let name = "test_name".to_string();
        let child_ref = ChildRef::new(quip_id, sender, name, path);

        assert!(!instance.actors.contains_key(&child_ref));
    ...
    fn test_global_dispatcher_add_local_dispatcher() {
        ...
        let local_dispatcher = Arc::new(Dispatcher::with_type(dispatcher_type.clone()));
        let global_dispatcher = GlobalDispatcher::new();

        assert!(!global_dispatcher.dispatchers.contains_key(&dispatcher_type));
    ...
    fn test_global_dispatcher_register_actor() {
        ...
        assert!(!local_dispatcher.actors.contains_key(&child_ref));
    ...
    fn test_global_dispatcher_notify() {
        ...
        let local_dispatcher =
            Arc::new(Dispatcher::with_type(dispatcher_type.clone()).with_handler(handler.clone()));
        let actor_groups = vec![dispatcher_type];
        let module_name = "my::test::module".to_string();
