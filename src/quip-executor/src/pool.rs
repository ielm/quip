...
use tinyproc::tinyproc::TinyProc;
use tracing::trace;
...
where
    ...
    T: Send + 'static,
{
    let (task, handle) = TinyProc::recoverable(future, worker::schedule, stack);
    task.schedule();
    handle
}
...
where
    ...
    R: Send + 'static,
{
    let (task, handle) = TinyProc::recoverable(future, schedule, stack);
    task.schedule();
    handle
}
...
pub(crate) fn schedule(t: TinyProc) {
...
#[derive(Debug)]
pub struct Pool {
    sender: Sender<TinyProc>,
    receiver: Receiver<TinyProc>,
}
